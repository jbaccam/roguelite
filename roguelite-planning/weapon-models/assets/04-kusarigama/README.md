...
Controls: select Kusarigama_Rig, enter Pose Mode, and rotate `Chain_01` through `Chain_11` from the held end toward the moving end. `Handle_ROOT` is fixed at the handhold; move the entire armature object to place the weapon. `Weight_END` rotates the terminal payload. Chain rotations move all downstream links and the payload. Translation/scale controls are locked to discourage opening joints or stretching links. Each whole link uses one weight of 1.0, with no blended-metal deformation.

The sickle and its hand grip are held; the chain begins at its bottom ferrule and ends at the weight.
